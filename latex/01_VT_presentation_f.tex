...
\begin{itemize}
  ...
  \item Microeconomic theory explains this productivity-wage gap (markdown) through monopsony power in the labor market.
  \item Labor markdowns represent not only foregone income for workers, but also a significant source of allocative inefficiency for the broader economy.
  \item Although monopsony power arises from structural features of labor markets, firms differ substantially in how aggressively they exploit it.
...
\textbf{This paper examines whether greater female leadership reduces the extent to which firms exploit monopsony power, i.e., whether it is associated with a lower productivity-wage gap.}
\end{frame}
...
\begin{itemize}
  ...
  \item 11,235 Colombian manufacturing firms observed over 2000–2019, yielding 53,688 firm-year observations spanning 105 industries and 23 geographic regions.
\end{itemize}
...
\begin{frame}{Markdown definition}

Firm's profit maximization under monopsony:

\begin{align}
\max_{L} \varPi = R(L) - \bar{w}L,
\end{align}

Inverse Elasticity Pricing Rule for a Monopsonist:

\begin{equation}
\frac{R'(L)-w(L)}{w(L)} = \frac{w'(L)L}{w(L)} \equiv \varepsilon_L^{-1}
\end{equation}

Rearranged following Yeh et al. (2022) we obtain the firm's markdown:
...
Following Yeh et al. (2022) and using the production approach by De Locker et al. (2020) we can obtain the firm's markdown as:
...
Where $\theta^L$ and $\theta^M$ are extracted from a translog production function GMM estimation:
\vspace{-0.5em}
\begin{flalign}
\begin{aligned}
...
\begin{itemize}
  ...
  \item Policies promoting gender diversity in executive and ownership positions (board gender quotas, bias-free promotion practices, and mentorship pipelines) may generate positive labor market externalities.
\end{itemize}
\end{frame}
